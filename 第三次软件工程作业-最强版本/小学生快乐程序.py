...
def get_Parameter():#命令行控制模块
    ...
    parser.add_argument('-n', help='设定题目数量', type=int,default=10)
    parser.add_argument('-r', help='设定数值范围', type=int,default=10)
...
def main():
    ...
    if args.r:
        ...
        Answers_list = list(range(0, n))
        for x in range(n):
            Answers_list[x] = '%s' % (get_Conversion(Answers[x]))
            Answers[x] = '%d. %s' % (x + 1, get_Conversion(Answers[x]))
        print('本次共生成题目%d道\n题目数值范围为0-%d' % (n, r))
        if os.path.exists('Exercises.txt'): os.remove('Exercises.txt')
        if os.path.exists('Answers.txt'): os.remove('Answers.txt')
        if os.path.exists('Answers.txt'): os.remove('Answers.txt')
        text_save('Exercises.txt', Exercises)
        text_save('Answers.txt', Answers)
        data1 = DataFrame(Answers_list)
        # 把对应题目的答案保存用于校对答案
        data1.to_csv('Answers_list.csv', index=False, header=True)
